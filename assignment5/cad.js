...
function Sphere(){
	...
	this.generateGeometry = function(){
		...
		var segments = 16;
		for(var i=0;i<Math.PI*2;i+=Math.PI/segments)
			for(var j=0;j<Math.PI;j+=Math.PI/segments)
...
function ImageTexture(gl){
	Texture.apply(this);
	this.properties['image'] = 'moon.png';
	...
	this.reload = function(){
		...
		image.src = this.properties['image'];
	}
}

function CheckerTexture(gl){
	Texture.apply(this);

	this.properties['size'] = 512;
	this.properties['checks'] = 4;
	...
	this.reload = function(){
		...
		var txSize = this.properties['size'];
		var checks = this.properties['checks'];

		var image = new Uint8Array(4*txSize*txSize);
		var c=0;
		
		for(var i=0; i<txSize; i++){
			for(var j=0; j<txSize; j++){
				var px = Math.floor(i/(txSize/checks));
				var py = Math.floor(j/(txSize/checks));
				...
				image[4*i*txSize+4*j] = c;
				image[4*i*txSize+4*j+1] = c;
				image[4*i*txSize+4*j+2] = c;
				image[4*i*txSize+4*j+3] = 255;
			}
		}
	    ...
	    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, txSize,txSize, 0, gl.RGBA, gl.UNSIGNED_BYTE, image);
	    gl.generateMipmap( gl.TEXTURE_2D );
	    gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST_MIPMAP_LINEAR );
	    gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
	}
...
function CAD(canvas){
	...
	var chkTx = new CheckerTexture(gl);
	chkTx.properties['size'] = 256;
	chkTx.properties['checks'] = 8;
	this.textures.push(chkTx);
	chkTx.reload();

	var textureNames = ['texture1.png','texture2.png','texture3.png','texture4.png'];
	for(var i in textureNames){
		var imgTx = new ImageTexture(gl);
		imgTx.properties['image'] = textureNames[i];
		this.textures.push(imgTx);
		imgTx.reload();
	}
	...
	this.render = function(){
		...
		for(var i in this.objects){
			...
			gl.uniform1f(gl.getUniformLocation(this.program,"fsuvtype"),object.normalMappingType);
			/*
			...
			*/
    		...
    		gl.activeTexture(gl.TEXTURE0);
  			gl.bindTexture(gl.TEXTURE_2D, this.textures[object.textureId].texture);
...
function createTextureProperties(textureList){
	var checkerSize = textureList[0].properties['size'];
	var checkerNumChecks = textureList[0].properties['checks'];

	var images = 

	$("#texture-list").append('\
		<h1>Slot 0 (Checker texture)</h1>\
		<div>\
			<p>Checks <input name="numChecks" type="range" min="2" max="32" value="'+checkerNumChecks+' "/></p>\
		</div>\
	');

	$("#texture-list input[name='numChecks']").change(function(event){
		var tx = textureList[0];
		tx.properties['checks'] = event.target.value;
		tx.reload();
	});
	$("#texture-list").append("<h1>Image textures</h1>");
	for(var i=1;i<textureList.length;i++){
		var tx = textureList[i];
		$("#texture-list").append('\
				<!--<h1>Slot '+i+' (Image texture)</h1>!-->\
				<img src = "'+tx.properties['image']+'" width="25%" alt="Slot '+i+' (Image texture)" title="Slot '+i+' (Image texture)"/>\
		');
	}
...
function createObjectProperties(object){
	...
	var normalMappingType = object.normalMappingType;
	var textureId = object.textureId;

	var texturelist = '';

	for(var i in cad.textures){
		var sel = object.textureId==i?'selected="selected"':'';
		texturelist+='<option value="'+i+'" '+sel+'>Slot '+i+'</option>';
	}
	...
	$("#object-properties").append('\
		<div class="property">\
			...
			<h1>Texture</h1>\
			<p>Use <select id="object-texture-id">'+texturelist+'</select></p>\
			<h1>UV Mapping</h1>\
			<p><input name="normal-mapping-type" type="radio" value="0.0" '+(normalMappingType<0.5?'checked="checked"':'')+'/>Regular</p>\
			<p><input name="normal-mapping-type" type="radio" value="1.0" '+(normalMappingType>0.5?'checked="checked"':'')+'/>Spherical</p>\
		</div>\
	');
	...
	$("#object-properties input[name='normal-mapping-type']").click(function(event){
		var type = $("#object-properties input[name='normal-mapping-type']:checked").val();
		console.log(type);
		object.normalMappingType = parseFloat(type);
	});
	$("#object-texture-id").change(function(event){
		var selected = parseInt(event.target.value);
		object.textureId = selected;
	});
}
...
function startCAD(canvasId){
	...
	createTextureProperties(cad.textures);
	...
	var tid = 0;
	for(var i=0;i<360;i+=360/cad.textures.length){
			var id = newObject('sphere');
			cad.objects[id].setPosition([Math.cos(radians(i))*5,0,Math.sin(radians(i))*5]);
			cad.objects[id].setScale([1.5,1.5,1.5]);
			cad.objects[id].textureId = tid++;
	}
	...
	setInterval(function(){
		aa+=1;
		cad.camera.position[0] = 5*Math.cos(radians(aa));
		cad.camera.position[2] = 5*Math.sin(radians(aa));
	},25);
}
